...
import { X, Search, Phone, MapPin, Clock, MessageSquare, Save, ChevronUp, ChevronDown } from "lucide-react";
...
const inputCls = "h-10 text-sm font-semibold bg-background/50 border-border/60 focus:border-primary/50";

export function DeliveryAddressModal({ customerInfo, onClose, onSave }: DeliveryAddressModalProps) {
  ...
  const handlePostcodeLookup = async () => {
    ...
    try {
      ...
      if (addresses.length === 1) {
        ...
        setTimeout(() => { houseNumberRef.current?.focus(); }, 80);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (withStatus?.status && withStatus.status !== 404) console.error("Verify address failed", err);
      onSave(formData);
    } finally {
      setIsLoading(false);
    }
  ...
  const actionStrip = (() => {
    if (focusedField === "phone") return (
      <button
        onMouseDown={(e) => { e.preventDefault(); handlePhoneLookup(); }}
    ...
    if (focusedField === "postcode") return (
      <button
        onMouseDown={(e) => { e.preventDefault(); handlePostcodeLookup(); }}
    ...
    if (focusedField === "houseNumber") return (
      <button
    ...
    >
      ...
      <div className="flex items-center justify-between border-b border-border/60 px-4 py-3 bg-muted/20 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-display text-base font-black tracking-tight uppercase">Customer Information</span>
          </div>
        </div>
        <div className="flex items-center gap-2">
          {formData.distance ? (
            <span className="font-mono text-sm font-bold text-accent">{formData.distance.toFixed(2)} mi</span>
          ) : null}
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-3 max-w-4xl mx-auto">
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button variant="secondary" size="icon" className="h-10 w-10 shrink-0" onClick={handlePhoneLookup} disabled={isLoading}>
          ...
          {/* Name */}
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Customer Name</label>
            <Input
              data-inline-keyboard="true"
              value={formData.name}
          ...
          <div className="space-y-1">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button variant="default" size="icon" className="h-10 w-10 shrink-0 shadow-md shadow-primary/20" onClick={handlePostcodeLookup} disabled={isLoading}>
                <Search className="h-4 w-4" />
              </Button>
            </div>
          </div>
          ...
          {/* Town */}
          <div className="space-y-1">
            <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Town / City</label>
            <Input
              data-inline-keyboard="true"
              value={formData.town}
          ...
          <div className="col-span-2 space-y-1">
            <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">House Number / Name</label>
            <Input
              ref={houseNumberRef}
              data-inline-keyboard="true"
          ...
          {/* Street — full width */}
          <div className="col-span-2 space-y-1">
            <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground">Street Name</label>
            <Input
              data-inline-keyboard="true"
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center justify-center gap-2 rounded-xl border border-border/60 bg-muted/30 p-2">
              ...
              <div className="flex flex-col items-center gap-1 flex-1">
                <button onClick={() => nudgeTime("hour", "up")} className="w-full flex justify-center rounded-lg py-1.5 hover:bg-muted active:bg-border transition-colors">
                ...
                <button onClick={() => nudgeTime("hour", "down")} className="w-full flex justify-center rounded-lg py-1.5 hover:bg-muted active:bg-border transition-colors">
              ...
              {/* Minute */}
              <div className="flex flex-col items-center gap-1 flex-1">
                <button onClick={() => nudgeTime("minute", "up")} className="w-full flex justify-center rounded-lg py-1.5 hover:bg-muted active:bg-border transition-colors">
                  <ChevronUp className="h-5 w-5 text-muted-foreground" />
                </button>
                <span className="font-mono text-2xl font-black text-foreground tabular-nums">
                  {formData.deliveryTime ? formData.deliveryTime.split(":")[1] : "--"}
                </span>
                <button onClick={() => nudgeTime("minute", "down")} className="w-full flex justify-center rounded-lg py-1.5 hover:bg-muted active:bg-border transition-colors">
                  <ChevronDown className="h-5 w-5 text-muted-foreground" />
                </button>
              </div>
            </div>
      ...
      <div className="shrink-0 border-t border-border/40 bg-muted/20 px-4 py-2">
        <div className="mx-auto max-w-4xl flex items-center gap-2">
          ...
          <Button variant="outline" className="h-11 px-5 font-semibold shrink-0" onClick={onClose} disabled={isLoading}>
            Cancel
          </Button>
          <Button className="h-11 px-6 font-black shadow-lg shadow-primary/20 gap-2 shrink-0" onClick={handleSave} disabled={isLoading}>
