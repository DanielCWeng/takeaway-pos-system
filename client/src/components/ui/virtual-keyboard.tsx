...
export function KeyboardPanel({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className={cn("flex flex-col", gap, pad)}>
      ...
      <div className={cn("flex justify-center", gap)}>
        <Key label="SPACE" onClick={(e) => handleKey("SPACE", e)} keyH={keyH} keyMin={keyMin} keyText={keyText} className="flex-1 max-w-md bg-muted/20" />
        <Key
          label="ENTER"
          icon={<CornerDownLeft className={compact ? "h-5 w-5" : "h-6 w-6"} />}
          onClick={(e) => handleKey("ENTER", e)}
          keyH={compact ? "h-12" : keyH}
          keyMin={keyMin}
          keyText={keyText}
          className={cn(compact ? "w-32 text-sm font-black" : "w-[120px]", "bg-primary text-primary-foreground hover:brightness-110 border-primary shadow-md shadow-primary/30")}
        />
      </div>
    </div>
  );
