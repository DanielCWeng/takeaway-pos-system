...
const KeyboardInputModal: React.FC<KeyboardInputModalProps> = ({
  ...
  const displayValue = inputType === "password" ? "•".repeat(inputValue.length) : inputValue;
    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
      <div className="bg-gray-300 p-4 border-4 border-t-gray-100 border-l-gray-100 border-b-gray-500 border-r-gray-500 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-2">
          <PosButton onClick={onClose} className="bg-red-400">
            Cancel
          </PosButton>
          <PosButton onClick={handleEnter} className="bg-green-400">
            Enter
          </PosButton>
        </div>
      </div>
