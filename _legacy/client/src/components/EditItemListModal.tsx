...
const EditItemListModal: React.FC<EditItemListModalProps> = ({
  ...
  const handleItemChange = (
    id: string,
    field: keyof MenuItem | "name.en" | "name.zh",
    value: any,
  ) => {
    setLocalItems(
      localItems.map((item) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
      <div className="bg-gray-800 rounded-lg shadow-xl w-full max-w-6xl h-[90vh] flex flex-col p-6">
        {" "}
        {/* Increased max-width */}
          ...
          <div className="flex flex-col overflow-hidden bg-gray-900 p-4 rounded-lg">
            ...
            <div className="flex gap-2 mb-2">
              ...
              <button
                ...
                className="bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-sm disabled:bg-gray-600"
              >
                Remove Item
              </button>
            </div>
                ...
                <tbody>
                  {localItems.map((item) => (
                    <tr
                      ...
                      className={`cursor-pointer ${selectedItemId === item.id ? "bg-blue-900" : "hover:bg-gray-700"}`}
                    >
                      ...
                      <td className="p-1">
                        <input
                          type="text"
                          value={item.name.en}
                          onChange={(e) => handleItemChange(item.id, "name.en", e.target.value)}
                      ...
                      <td className="p-1">
                        <input
                          type="text"
                          value={item.name.zh}
                          onChange={(e) => handleItemChange(item.id, "name.zh", e.target.value)}
                          className="bg-transparent w-full"
                        />
                      </td>
                      <td className="p-1">
                      ...
                      <td className="p-1">
                        <input
                          ...
                          onChange={(e) =>
                            handleItemChange(item.id, "primaryCategory", e.target.value)
                          }
                          className="bg-transparent w-full"
                        />
                      </td>
                      <td className="p-1">
                        <input
                          ...
                          onChange={(e) =>
                            handleItemChange(item.id, "secondaryCategory", e.target.value)
                          }
                          className="bg-transparent w-full"
                        />
                      </td>
