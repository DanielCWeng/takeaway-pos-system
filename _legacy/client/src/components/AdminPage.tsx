    ...
    <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-[60]">
      <div className="bg-gray-200 text-black rounded-lg shadow-xl w-full max-w-lg flex flex-col">
        ...
        <div className="p-4 space-y-2 flex-grow overflow-y-auto max-h-[60vh]">
          ...
          <p>
            <strong>Address:</strong> {order.customerInfo.address || "N/A"}
          </p>
          <hr className="my-2" />
...
const DeleteConfirmationModal: React.FC<{
  ...
  onClose: () => void;
  onConfirm: () => void;
}> = ({ orderCount, date, onClose, onConfirm }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-[70]">
      <div className="bg-white text-black p-6 rounded-lg shadow-xl flex flex-col gap-4 w-96">
        ...
        <p>
          Are you sure you want to permanently delete all <strong>{orderCount}</strong> orders for{" "}
          <strong>{date}</strong>?
          <br />
    ...
    <>
      <div className="fixed inset-0 bg-gray-800 text-white z-50 p-4 flex flex-col">
        <div className="flex-shrink-0 flex justify-between items-center mb-4">
          ...
          <button onClick={onClose} className="p-2 bg-red-600 rounded-md hover:bg-red-700">
            Close Admin
          </button>
        </div>
        ...
        <div className="flex-shrink-0 bg-gray-700 p-4 rounded-md flex flex-col md:flex-row justify-between items-center gap-4 mb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <label htmlFor="date-selector" className="mr-2">
                Date:
              </label>
              <input
                type="date"
            ...
            <input
              ...
              className="bg-gray-800 p-1 rounded-md w-64"
            />
          </div>
          <div className="flex items-center gap-4">
            {" "}
            {/* [NEW] Flex container for total and delete button */}
            ...
            <button
              ...
              className="px-3 py-1 bg-red-700 text-white rounded-md hover:bg-red-800 disabled:bg-gray-500 disabled:cursor-not-allowed"
            >
              Delete All
            </button>
          </div>
              ...
              <tbody>
                {filteredOrders.map((order) => (
                  ...
                  >
                    ...
                    <td className="p-2">
                      {order.customerInfo.name || order.customerInfo.phone || "N/A"}
                    </td>
                    <td className="p-2 text-sm text-gray-400 hidden md:table-cell">
                      {order.items.map((i) => i.displayName).join(", ")}
                    </td>
