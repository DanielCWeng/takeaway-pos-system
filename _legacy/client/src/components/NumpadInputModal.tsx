...
const NumpadInputModal: React.FC<NumpadInputModalProps> = ({
  title,
  initialValue = "",
  onClose,
  onConfirm,
  ...
  return (
    ...
    >
      <div
        className="bg-gray-200 p-4 border-4 border-t-white border-l-white border-b-gray-600 border-r-gray-600 shadow-xl flex flex-col gap-4 w-80"
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-2">
          {["7", "8", "9"].map((key) => (
            <PosButton key={key} onClick={() => handlePress(key)} className="h-14">
              {key}
            </PosButton>
          ))}
          {["4", "5", "6"].map((key) => (
          ...
          <PosButton onClick={() => handlePress("0")} className="h-14">
            0
          </PosButton>
          <PosButton onClick={() => handlePress(".")} className="h-14">
            .
          </PosButton>
          <PosButton onClick={() => handlePress("Back")} className="h-14">
            ⌫
          </PosButton>
        </div>
        ...
        <PosButton
          ...
          className="h-14 mt-2 bg-green-600 text-white border-l-green-400 border-t-green-400 border-r-green-800 border-b-green-800 active:bg-green-700"
        >
          ENTER
        </PosButton>
      </div>
